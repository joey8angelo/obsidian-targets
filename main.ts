...
export interface Settings {
	...
	targets: (WordCountTarget | TimeTarget)[];
	maxIdleTime: number;
	lastReset: Date;
	progressHistory: {
		daily: {
...
export default class TargetTracker extends Plugin {
	...
	private async loadSettings() {
		console.log("obsidian-targets: loading settings");
	...
	private async saveSettings() {
		console.log("obsidian-targets: saving settings");
	...
	async onload() {
		...
		this.registerView(
			VIEW_TYPE_TARGET,
			(leaf) => new TargetView(leaf, this),
		);

		this.addRibbonIcon("dice", "Open target view", () => {
			this.activateView();
		});
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				...
				dropdown.setValue(
					this.plugin.settings.dailyResetHour.toString(),
				);
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				...
				dropdown.setValue(
					this.plugin.settings.weeklyResetDay.toString(),
				);
				dropdown.onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addText((text) => {
				text.setPlaceholder("Enter max idle time in seconds").setValue(
					(this.plugin.settings.maxIdleTime / 1000).toString(),
				);
