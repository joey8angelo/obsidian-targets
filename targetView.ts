...
import { TimeTarget, WordCountTarget } from "./target";
...
export class TargetView extends ItemView {
	...
	private renderDisplayTarget(
		...
		editButton.onclick = () => {
			this.editingStates.set(target.id, {
				...
				multiplier:
					target instanceof TimeTarget ? target.multiplier : 1,
			} as EditingState);
			this.renderContent();
		};
		...
		archiveButton.onclick = () => {
			if (
				confirm(
					`Are you sure you want to delete target "${target.name}"?`,
				)
			) {
	...
	private renderEditingTarget(
		...
		const targetInputEl = targetInputContainer.createEl("input", {
			...
			value:
				target instanceof WordCountTarget
					...
					: (
							editingState.target /
							(target as TimeTarget).multiplier
						).toString(),
		});
		...
		if (target instanceof TimeTarget) {
			...
			targetDurationSelect.onchange = (e) => {
				...
				editingState.target =
					parseInt(targetInputEl.value) * multiplier;
			};
		}
	...
	renderContent() {
		...
		addWordCountButton.onclick = () => {
			const target = this.plugin.targetManager.newWordCountTarget();
			this.editingStates.set(target.id, {
				name: target.name,
				path: target.path,
				period: target.period,
		...
		addTimeButton.onclick = () => {
			const target = this.plugin.targetManager.newTimeTarget();
			this.editingStates.set(target.id, {
				name: target.name,
				path: target.path,
				period: target.period,
